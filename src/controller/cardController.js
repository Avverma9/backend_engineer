...
const createCard = async function (req,res){
    ...
    try{
        ...
        if(!ValidName(customerName.trim())){ return res.status(400).send ({ status : false, message : "Customer Name Should be in alphabet"})}
      
        if(!status) { return res.status(400).send ({ status : false, message : "Please give your card status"})}
        if(status !="ACTIVE" && status != "INACTIVE"){
            return res.status(400).send({status:false,message:"status is only acceptable in ACTIVE OR INACTIVE"})
        }
